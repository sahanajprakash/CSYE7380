...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 shadow-xl text-xs dark:border-slate-700 dark:bg-slate-900">
      ...
      <p className="text-emerald-600 dark:text-emerald-400">Equity: {payload[0]?.value?.toFixed(4)}</p>
      {payload[1] && (
        <p className="text-red-600 dark:text-red-400">Drawdown: {(payload[1].value * 100).toFixed(2)}%</p>
      )}
    </div>
  );
}
